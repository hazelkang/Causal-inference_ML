...
data        <- read.dta13("centrality_clean3.dta")
...
names <- c("IPOMT")    # vector of labels for outcome variables
Y     <- c("n_mt")     # vector of outcome variables
D     <- rep("highpeer_inter_cost_w_s", length(Y))  # vector of treatment variables

# specify cluster, fixed effect and partition
cluster      <- "sch_id"       # if no cluster       use    cluster      <- "0"
fixed_effect <- "sch_id"         # if no fixed_effect  use    fixed_effect <- "0"
partition    <- "0"       # if no partition     use    partition    <- "0"

# create a vector of control variables
controls     <- c("incpercap", "parent_age", "parent_edu", "electricity", "female", "IPEMT_peer", "mat_base", "inter_cost_w_s")

affected       <- c("incpercap", "parent_age", "parent_edu", "electricity", "female", "IPEMT_peer", "mat_base", "inter_cost_w_s")
      # characteristics for most/least affected analysis
names_affected       <- c("incpercap", "parent_age", "parent_edu", "electricity", "female", "IPEMT_peer", "mat_base", "inter_cost_w_s")
